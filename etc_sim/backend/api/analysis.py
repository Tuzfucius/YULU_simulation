...
async def get_chart_data(
    simulation_id: str,
    chart_type: str,
    time_range: Optional[str] = Query(None, pattern=r"^\d+-\d+$"),
    segments: Optional[List[int]] = Query(None),
) -> Dict[str, Any]:
    """获取图表数据。"""
    del time_range, segments
    valid_charts = [
        "speedHeatmap",
        "trajectory",
        "anomalyDistribution",
...
def generate_mock_chart_data(chart_type: str) -> dict:
    ...
    data_generators = {
        ...
        "fundamentalDiagram": lambda: {
            ...
            "flow": [density * (80 - density) / 5 for density in range(0, 80, 5)],
            "speed": [80 - density * 0.8 for density in range(0, 80, 5)],
        },
    }
